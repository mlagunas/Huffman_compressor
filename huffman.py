...
# It returns the compressed code and its codigied tree for a given string
# (inputFile)


def preCompression(input):
    itemqueue = [Node(a, len(list(b))) for a, b in groupby(
        sorted(input))]  # input sorted and grouped by character
    # nElements in binary an 8 digits
    count = '{0:08b}'.format(len(itemqueue) - 1)
...
def huffmanCompression(inputFile, outFile):
    ...
    if benchmark:
        ...
        print "Precompress (tree + binary codes) " + str(end - start)
    ...
    outf.write(bOut)  # Get number of written bytes
    outf.close()
    inf.close()
...
def decodeIt2(input, node, count, code=""):

    if node.item:
        return node.item, code, count
    else:
        if input[count] == '0':
            return decodeIt2(input, node.left,  count + 1, code + '0')
        else:
            return decodeIt2(input, node.right,  count + 1, code + '1')
...
def preDecompression2(input, tree):
    s = ""
    # doH = len(input) > 80000
    doH = False
    index = 0
    if len(input) == 1:
        char, code, i_aux = decodeIt2(input, tree, index)
        index = i_aux
        s += char
    else:
        while len(input) > index:
            char, code, index = decodeIt2(input, tree, index)
            s += char
    return s
...
def buildTree2(input, count, index, node=None, root=None):
    if count > 0:
        if input[index] == '0':  # not final-leaf node
            if not node:  # create root
                root = Node(None, 0)
                return buildTree2(input, count, index + 1, root, root)
            else:
                if node.left and node.right:  # if node has both children go to node parent and check again
                    return buildTree2(input, count, index, node.parent, root)
                else:
                    n = Node(None, 0)
                    n.setParent(node)
                    if node.left:  # If left children exits new node has to be righ children
                        ...
                        node.setLeft(n)
                    return buildTree2(input, count, index + 1, n, root)
        else:  # final-leaf node
            if not node:  # create root with item value
                count -= 1
                root = Node(chr(int(input[index + 1:index + 9], 2)), 0)
                return buildTree2(input, count, index + 9, root, root)
            elif node.left and node.right:  # if node has both children go to node parent and check again
                return buildTree2(input, count, index, node.parent, root)
            else:
                count -= 1
                n = Node(chr(int(input[index + 1:index + 9], 2)), 0)
                n.setParent(node)
                if node.left:  # If left children exists new node has to be righ children
                    node.setRight(n)
                else:  # If there are no nodes, new node has to be left children
                    node.setLeft(n)
                return buildTree2(input, count, index + 9, node, root)
    else:
        return root, input[index:]
...
def huffmanDecompression(inputFile, outFile):
    ...
    if benchmark:
        ...
        print "Format huf " + str(end - start)

    if benchmark:
        start = time.time()
    # Get the tree and rest of the code to decompress
    tree, compression = buildTree2(code[8:], int(code[:8], 2) + 1, 0)
    if benchmark:
        end = time.time()
        print "Build tree " + str(end - start)
    ...
    # Decompress the code
    if count != 0:
        res = preDecompression2(compression[:-count], tree)
    else:
        res = preDecompression2(compression, tree)
    if benchmark:
        end = time.time()
        print "Get codes " + str(end - start)
